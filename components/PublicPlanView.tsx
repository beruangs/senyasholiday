...
export default function PublicPlanView({ planIdOrSlug }: { planIdOrSlug: string }) {
  const searchParams = useSearchParams()
  const planId = planIdOrSlug
  ...
  const isPremium = plan?.planIsPremium || false
  ...
  const openDayRoute = (date: string) => {
    ...
    const dayItems = groupedRundowns[date]
    const locations = dayItems.map((item: any) => item.location).filter(Boolean)
  ...
  const themeStyles = plan?.theme === 'custom' ? {
    '--primary': plan.customPrimaryColor,
    '--primary-dark': plan.customPrimaryColor,
    '--tw-shadow-color': `${plan.customPrimaryColor}33`,
    '--bg-primary-50': `${plan.customPrimaryColor}10`,
  } as React.CSSProperties : {}

  return (
    <div className="min-h-screen bg-gray-50/50 font-bold px-4 print:px-0" data-plan-theme={plan?.theme || 'default'} style={themeStyles}>
      {/* ==========================================
                  ...
                  <div key={date} className="relative print:break-inside-avoid print:mt-8">
                    <div className="flex items-center justify-between mb-8">
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-xl hover:bg-black transition-all font-black text-[9px] uppercase tracking-widest group shadow-xl"
                      >
                        <MapPin className="w-3.5 h-3.5 text-primary-400" />
                        <span>{language === 'id' ? 'LIHAT RUTE' : 'DAY ROUTE'}</span>
                        {!isPremium && <Sparkles className="w-3 h-3 text-amber-500 fill-amber-400" />}
                      </button>
                    </div>
                              ...
                              <div className="flex flex-wrap gap-6">
                                {rundown.location && (
                                  <button
                                    ...
                                    className={`text-[11px] font-black uppercase tracking-widest flex items-center gap-2 transition-all ${isPremium ? 'text-primary-600 hover:underline' : 'text-gray-500'}`}
                                  >
                                    <MapPin className={`w-4 h-4 ${isPremium ? 'text-primary-600' : 'text-primary-400'}`} />
                                    {rundown.location}
                                  </button>
                                )}
                        ...
                        <tr key={expense._id} className="print:break-inside-avoid">
                          ...
                          <td className="px-6 md:px-10 py-6 text-right">
                            <div className="flex flex-col items-end">
                              <span className="font-black text-gray-900 text-sm">{formatCurrency(expense.total)}</span>
                              {isPremium && expense.currency && expense.currency !== 'IDR' && (
                                <span className="text-[8px] font-bold text-gray-400 uppercase tracking-tighter">
                                  {expense.originalPrice} {expense.currency} (@{expense.exchangeRate})
                                </span>
                              )}
                            </div>
                          </td>
