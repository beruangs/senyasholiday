...
export default function ParticipantsTab({ planId }: { planId: string }) {
  ...
  const handleBulkSubmit = async (e: React.FormEvent) => {
    ...
    if (names.length === 0) {
      toast.error('Masukkan minimal 1 nama peserta')
      return
    }
    ...
    try {
      ...
      const promises = names.map((name, index) =>
        fetch('/api/participants', {
          ...
          body: JSON.stringify({
            holidayPlanId: planId,
            name,
            order: participants.length + index,
          }),
        })
      )
      ...
      if (failedCount > 0) {
        toast.warning(`${successCount} peserta berhasil ditambahkan, ${failedCount} gagal`)
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      {showBulkForm && (
        <form onSubmit={handleBulkSubmit} className="bg-green-50 rounded-lg p-6 border-2 border-green-200">
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Tambah Banyak Peserta Sekaligus <span className="text-red-500">*</span>
            </label>
            <p className="text-xs text-gray-600 mb-2">
              💡 Tulis satu nama per baris. Contoh:
            </p>
            <div className="bg-white p-2 rounded border border-gray-200 text-xs text-gray-600 mb-3">
              Jeje<br/>
              Wafi<br/>
              Abim<br/>
              Radit
            ...
            <textarea
              ...
              className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none font-mono text-sm"
              placeholder="Tulis nama peserta di sini, satu nama per baris..."
              rows={10}
              disabled={isSubmitting}
            />
            <p className="text-xs text-gray-600 mt-2">
              {bulkNames.split('\n').filter(n => n.trim().length > 0).length} peserta akan ditambahkan
            </p>
          </div>
          <div className="flex justify-end space-x-3">
            ...
            <button
              ...
              disabled={isSubmitting}
            >
              {isSubmitting ? 'Menambahkan...' : 'Tambahkan Semua'}
            </button>
          </div>
        </form>
      )}
