...
export default function DigitalReceipt({ bill, onClose, language }: DigitalReceiptProps) {
    ...
    return (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300 font-bold">
            <div className="relative w-full max-w-lg bg-white rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-300">
                <div className="bg-primary-600 px-10 pt-12 pb-20 text-center text-white relative">
                    <button onClick={onClose} className="absolute top-8 right-8 p-2 bg-white/10 hover:bg-white/20 rounded-full transition-all"><X className="w-5 h-5" /></button>
                    <div className="w-16 h-16 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center mx-auto mb-6 border border-white/30"><Receipt className="w-8 h-8 text-white" /></div>
                    <h2 className="text-2xl font-black uppercase tracking-tight mb-1">{bill.title}</h2>
                ...
                <div className="flex-1 overflow-y-auto px-10 py-10 space-y-10 no-scrollbar">
                    <div className="flex items-center justify-between p-6 bg-gray-50 rounded-[2rem] border border-gray-100">
                        <div className="flex items-center gap-4">
                            ...
                            <div><span className="text-[8px] font-black uppercase text-gray-400 tracking-widest">{language === 'id' ? 'PENERIMA DATA' : 'RECEIVER'}</span><p className="text-sm font-black text-gray-900 uppercase tracking-tight">{bill.payerId?.name}</p></div>
                        </div>
                    ...
                    <div>
                        ...
                        <div className="space-y-6">{bill.items.map((item: any, idx: number) => (
                            <div key={idx} className="group">
                                ...
                                <div className="flex flex-wrap gap-1.5">{item.involvedParticipants.map((p: any) => (<span key={p._id} className="px-2 py-0.5 rounded-md bg-gray-50 text-gray-500 text-[8px] font-black uppercase tracking-tighter border border-gray-100">{p.name}</span>))}</div>
                    ...
                    <div className="bg-gray-900 rounded-[2.5rem] p-10 text-white shadow-2xl">
                        ...
                        <div className="flex justify-between items-center mb-10"><span className="text-xl font-black uppercase tracking-tighter">Grand Total</span><span className="text-3xl font-black text-primary-400">{formatCurrency(bill.totalAmount)}</span></div>
                ...
                <div className="bg-gray-50 border-t border-gray-100 p-8 text-center"><p className="text-[8px] font-black uppercase text-gray-300 tracking-[0.4em]">Receipt powered by S.E.N Holiday App</p></div>
            </div>
        </div>
    )
}
