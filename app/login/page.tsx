    ...
    <div className="min-h-screen bg-white flex items-center justify-center p-6 font-bold relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-12 animate-in slide-in-from-top-4 duration-700">
          ...
          <p className="text-[10px] font-black text-primary-600 tracking-[0.4em]">{t.auth.login_subtitle}</p>
        </div>

        <div className="bg-white rounded-[3.5rem] p-12 border border-gray-100 shadow-2xl animate-in zoom-in-95 duration-500">
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="space-y-3"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t.auth.username}</label><div className="relative group"><AtSign className="absolute left-6 h-4 w-4 text-gray-300 top-1/2 -translate-y-1/2 group-focus-within:text-primary-600 transition-all" /><input type="text" value={username} onChange={e => setUsername(e.target.value)} className="w-full pl-14 pr-8 py-5 bg-gray-50 border border-gray-100 rounded-3xl outline-none font-black text-gray-900 focus:bg-white focus:border-primary-500 transition-all tracking-tight" placeholder="Username" required /></div></div>
            <div className="space-y-3"><label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t.auth.password}</label><div className="relative group"><Lock className="absolute left-6 h-4 w-4 text-gray-300 top-1/2 -translate-y-1/2 group-focus-within:text-primary-600 transition-all" /><input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full pl-14 pr-8 py-5 bg-gray-50 border border-gray-100 rounded-3xl outline-none font-black text-gray-900 focus:bg-white focus:border-primary-500 transition-all tracking-tight" placeholder="••••••••" required /></div></div>
